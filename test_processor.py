...
LOG_VALIDO_GET  = '192.168.0.1 - - [27/Jul/2026:14:32:10 +0000] "GET /api/v1/users HTTP/1.1" 200 3421\n'
LOG_VALIDO_POST = '10.0.0.5 - - [27/Jul/2026:15:00:00 +0000] "POST /login HTTP/1.1" 201 500\n'
...
def arquivo_mock():
    ...
    with os.fdopen(fd, 'w') as f:
        f.write(LOG_VALIDO_GET)
        f.write(LOG_VALIDO_POST)
        f.write(LOG_LIXO)
    yield path
    os.remove(path)
...
def test_descarte_linhas_invalidas_com_drop_nulls(arquivo_mock):
    ...
    assert df.height == 2, "O DataFrame deveria conter 2 linhas válidas (GET e POST) após o drop_nulls()"


def test_regex_aceita_metodos_diversos():
    """Garante que o parser aceita POST, PUT e DELETE além de GET."""
    logs = (
        '192.168.0.1 - - [27/Jul/2026:14:32:10 +0000] "POST /login HTTP/1.1" 201 500\n'
        '192.168.0.1 - - [27/Jul/2026:14:32:10 +0000] "PUT /api/v1/users HTTP/1.1" 200 100\n'
        '192.168.0.1 - - [27/Jul/2026:14:32:10 +0000] "DELETE /api/v1/users/1 HTTP/1.1" 204 0\n'
    )
    fd, path = tempfile.mkstemp()
    with os.fdopen(fd, 'w') as f:
        f.write(logs)
    try:
        ...
        assert df.height == 3, "O parser deveria aceitar POST, PUT e DELETE"
    finally:
        os.remove(path)
...
def test_conversao_data_dt_partition(arquivo_mock):
    ...
    assert resultado["dt_partition"] == data_esperada, (
        f"A data da partição deveria ser {data_esperada}, mas retornou {resultado['dt_partition']}"
    )
...
def test_filtro_qualidade_dados_ip_status():
    ...
    log_ip_octeto_alto = '999.999.999.999 - - [27/Jul/2026:14:32:10 +0000] "GET /api/v1/users HTTP/1.1" 200 3421\n'
    ...
    with os.fdopen(fd, 'w') as f:
        ...
        f.write(log_ip_octeto_alto)
        f.write(log_status_invalido)

    try:
        df = process_logs(path)
        assert df.height == 1, "O filtro de qualidade falhou ao descartar IPs ou Status inválidos"
    finally:
        os.remove(path)
